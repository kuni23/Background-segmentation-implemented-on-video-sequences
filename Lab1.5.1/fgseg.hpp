...
namespace fgseg {
	...
	public:
		...
		bgs(int K, double dev_weight, double W_th, double alpha, bool rgb);
		...
		void removeShadows(cv::Mat Frame);

		//Normalize the weights
		void normalize(int i, int j);

		//Making the ordering in weights and means based on the weights as descending
		void ordering(int i, int j);

		bool IsFore(int a, int b, int k);
	...
	private:
		...
		//Storing the weights, means, three of them should have the same size!!
		Mat_<double> weights[4];
		Mat_<double> means[4];
		Mat_<double> devs[4];
